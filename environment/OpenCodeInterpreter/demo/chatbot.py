...
JSON_DATASET_DIR = Path("/app/logs/json_dataset")
...
def save_json(dialog, mode, json_file_path, dialog_id) -> None:
    # Removed scheduler.lock to save locally
...
def gradio_launch(input_prompt:str , model_path: str, MAX_TRY: int = 3):
    with gr.Blocks() as demo:
        ...
        def bot(user_message, history, jupyter_state, dialog_info, interpreter):# this function is used to deal with user input 
            logging.info(f"user message: {user_message}")# this will receive user message
            ...
            while HAS_CODE:
                ...
                if has_input_function_calls(generated_code_block):
                    ...
                    (
                        code_block_output,
                        error_flag,
                    ) = interpreter.execute_code_and_return_output( # Here is running the code!!!
        ...
        msg = "write python to sum a and b"
        msg = input_prompt
        ## here the input msg is added by chengquan, 
        # later this input msg can be passed by us in program diretly!
        gen = bot(msg, session_state, jupyter_state, dialog_info, interpreter)
        # (bot funtion deal with input and generate code, execute code) 
        # here we call bot function directly,no need to use gradio app to mouse click
        for result in gen:#
            tmp = 1
            # print("bot is running")
            # print(result)# see what we get, though needless
        print("finish")
        sys.exit()# our goal aleardy finished, exit
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Prompt to the OpenCodeInterpreter Model.",
        default="no input",
    )
    args = parser.parse_args()

    gradio_launch(input_prompt=args.input_prompt, model_path=args.path)
